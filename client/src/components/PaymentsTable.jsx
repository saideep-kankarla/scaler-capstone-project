...
export default function UsersTable() {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${apiBaseUrl}/api/payments/getAllPayments`,
        );
  ...
  const handleDelete = async () => {
    try {
      ...
      await Promise.all(
        selected.map((id) =>
          axios.delete(`${apiBaseUrl}/api/payments/deletePayment/${id}`),
        ),
      );
      setData((prevData) =>
        ...
        <Table>
          <TableHead>
            <TableRow className="">
              ...
              <TableCell>Payment done on</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {data.map((row) => (
              <TableRow key={row._id}>
                ...
                <TableCell>{row.stripePaymentId}</TableCell>
